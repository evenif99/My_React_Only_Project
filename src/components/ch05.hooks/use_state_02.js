...
function App() {
    ...
    /* 관리해야 할 상태(state) 정의 */ /* [state, setter메소드] = useState() */
    const [color, setColor] = useState('blue'); // 차량 색상 영문 이름
    const [year, setYear] = useState(2024); // 차량 생산년도
    ...
    const [modelName, setModelName] = useState('아반테(avante)'); // 한글 이름과 함께 보여줄 state

    // 중첩 배열을 사용한 Map 객체 정의
    const carMap = new Map([ // 자바 맵함수 복습할 것!!
        ['avante', '아반떼'],
        ['sonata', '소나타'],
        ['grandeur', '그랜져']
    ]);
